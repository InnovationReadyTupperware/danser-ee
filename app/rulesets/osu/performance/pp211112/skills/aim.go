...
import (
	...
	preprocessing2 "github.com/innovationreadytupperware/danser-ee/app/rulesets/osu/performance/pp211112/preprocessing"
...
func (skill *AimSkill) aimStrainValue(current *preprocessing2.DifficultyObject) float64 {
		...
		if !math.IsNaN(osuCurrObj.Angle) && !math.IsNaN(osuLastObj.Angle) && !math.IsNaN(osuLastLastObj.Angle) {
			...
			if osuCurrObj.StrainTime > 100 { // Only buff deltaTime exceeding 300 bpm 1/2.
				...
				acuteAngleBonus *= calcAcuteAngleBonus(lastAngle) * // Multiply by previous angle, we don't want to buff unless this is a wiggle type pattern.
					...
					math.Pow(math.Sin(math.Pi/2*(mutils.Clamp(osuCurrObj.JumpDistance, 50, 100)-50)/50), 2) // Buff distance exceeding 50 (radius) up to 100 (diameter).
			}
	...
	if max(prevVelocity, currVelocity) != 0 {
		...
		nonOverlapVelocityBuff := math.Abs(prevVelocity-currVelocity) *
			...
			math.Pow(math.Sin(math.Pi/2*min(1, min(osuCurrObj.JumpDistance, osuLastObj.JumpDistance)/100)), 2)
	...
	aimStrain += max(acuteAngleBonus*acuteAngleMultiplier, wideAngleBonus*wideAngleMultiplier+velocityChangeBonus*velocityChangeMultiplier)
